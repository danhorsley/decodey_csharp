...
namespace Decodey.Views
{
    public partial class SettingsDialog : ContentPage
    {
        ...
        {
            // Note: InitializeComponent() is generated by XAML compilation
            // and will be available at runtime if the XAML file exists
        ...
        {
            // Get the value as float (0.0 to 1.0)
            float volumeValue = (float)e.NewValue;

            // Update sound service
            _soundService?.SetVolume(volumeValue);

            // Play sound to test volume
            if (volumeValue > 0 && _viewModel.SoundEnabled &&
